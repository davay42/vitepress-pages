export default function (metaData: {
  ...
  return function transformHead(ctx) {
    ...
    if (ctx.pageData.frontmatter?.cover) {
      image = `${metaData.mediaFolder || 'media_files'}/cover/${url.split('/').join('-') + ctx.pageData.frontmatter?.cover}`
    }
    return [
      ...
      metaData.icon ? ["link", { rel: "icon", type: "image/svg+xml", href: metaData.icon }] : null,
      ...
      metaData.title ? ['meta', { name: 'twitter:title', content: `${ctx.pageData.title} | ${metaData.title}` }] : null,
